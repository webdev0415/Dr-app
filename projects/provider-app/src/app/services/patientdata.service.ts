...
import {catchError, concatMap, filter, first, map, pluck, skip, takeUntil} from 'rxjs/operators';
...
import { DDXRequest, IllnessesInformation, IllnessesPOSTRequest } from 'common/interfaces/diagnostic-engine.interface';
import { AddedExamViewModel } from '../common/interfaces/physical-exams.interfaces';
import { physicalExamDesc, telemedExamPIC } from '../physical-exam/physical-exams.constants';
...
import { AdditionalDoctorNotes, DDX, RawDDXAnalysis } from 'common/models/additional-doctor-notes.interface';
...
export class PatientdataService {
  private Patient: BehaviorSubject<DataModel.Data | null | any> = new BehaviorSubject<DataModel.Data | null | any>(null);
  private PatientVal: BehaviorSubject<DataModel.Data | null | any> = new BehaviorSubject<DataModel.Data | null | any>(null);
  private VisitData: BehaviorSubject<any> = new BehaviorSubject<any>(null);
  private _visitDataContext: BehaviorSubject<{[key in keyof Data]: string}> = new BehaviorSubject<{[key in keyof Data]: string}>(null);
  ...
  private differentialDiagnosis: BehaviorSubject<DDX> = new BehaviorSubject<DDX>(null);
  private isPatientLoaded: BehaviorSubject<boolean> = new BehaviorSubject<boolean>(false);
  ...
  public getPatient(): Observable<DataModel.Data> { return this.Patient.asObservable(); }
  public getPatientVal(): Observable<DataModel.Data> { return this.PatientVal.asObservable(); }
  public getPatientLastValue(): DataModel.Data | null { return this.Patient.getValue(); }
  public isPatientLoaded$(): Observable<boolean> { return this.isPatientLoaded.asObservable(); }
  public getVisitData(): Observable<any> { return this.VisitData.asObservable(); }
  public getHPISummary(): Observable<any> { return this.Summary.asObservable(); }
  ...
  public getDifferentialDiagnosis(filterValid = true): Observable<DDX> {
    return this.differentialDiagnosis.asObservable().pipe(map((ddx: DDX) => {
      ...
      if (!filterValid || !patient || !ddx) return ddx;
        ...
        if (Object.keys(ddx).some(icdCode => {
          const DE = diagnosticEngine.find(item => item.icd10 === icdCode);
  ...
  getPatientData(id) {
    ...
    forkJoin(
      ...
      if (!body.patientHealthHistory.historyItem) {
        this.notificationService.error('The patient\'s data format is not supported.', 'Unable to open patient.');
      ...
      if (!body.patientHealthHistory.historyItem.length) {
        ...
        if (this.userService.getUserRole !== UserRolesEnum.PRACTITIONER) this.notificationService.warning('Health history is missing.');
      }
      ...
      this.lazyLoader.lazyLoadMeasurements(body['measurements'])
        .subscribe(loadedMeasurements => this.measurementsMediaService.setMeasurements(loadedMeasurements));

      this.lazyLoader.lazyLoadIdCards(cards).then(loadedCards => {
        if (!loadedCards.length) return;
        this.getPatient().pipe(first(data => !!data), map(data => data.patientInformation)).subscribe(patientInformation => {
          patientsCards.forEach(key => {
            if (patientInformation[key]) {
              patientInformation[key].images = patientInformation[key].images.map(item => {
                const blobbedItem = loadedCards.find(image => image.side === item.side && image.card === key);
  ...
  private setUpPatient(patient: DataModel.Data): DataModel.Data {
    ...
    if (patient.additionalInformation.sportsPhysical && !patient.additionalInformation.sportsPhysical.length) {
      patient.additionalInformation.sportsPhysical = defaultPatient.additionalInformation.sportsPhysical;
      Object.assign(patient.additionalInformation.sportsCleared, defaultPatient.additionalInformation.sportsCleared);
    }
    patient.additionalInformation = Object.assign(defaultPatient.additionalInformation, patient.additionalInformation);
    patient.orders = this.orders;
    patient = Object.assign(defaultPatient, patient);

    patient.patientHealthHistory.historyItem.filter(item => item.symptomID === HistoryTypesEnums['MEDICATION-ALLERGIES']).forEach(item => {
      const medication = (item.historyItem as MedicationData)?.name || '';
      const commonName = drugNameMap[medication.toUpperCase()] || medication.toUpperCase();
      patient.additionalInformation.knownDrugAllergies.push({conflict: medication, potentialDrugs: [commonName]});
    });

    patient.patientHealthHistory.historyItem.filter(item => item.symptomID === HistoryTypesEnums.MEDICATION).forEach(item => {
      const medication = (item.historyItem as MedicationData)?.name || '';
      const commonName = drugNameMap[medication.toUpperCase()] || medication.toUpperCase();
      patient.additionalInformation.knownDrugConflicts.push({conflict: medication, potentialDrugs: [commonName]});
    });
    ...
    if (patientViewOnly) {
      ...
      const [images, audio] = [
        this.measurementsMediaService.getMediaByType(patient.measurements, 'images'),
        this.measurementsMediaService.getMediaByType(patient.measurements, 'audio')
      ];

      // todo: decide what to do with patients those had video, but it was disappear
      if (Array.isArray(images) && images.length && images.every(item => item.data.length)) images.forEach(item => {
      ...
      if (Array.isArray(audio)) audio.forEach(item => {
        switch (item.bodyPart) {
          case 'heart': patient.mediaReviewed.Cardiovascular = false; break;
          case 'lung': patient.mediaReviewed.Respiratory = false; break;
          case 'abdomen': patient.mediaReviewed.GI = false; break;
        }
      });
  ...
  public updatePatient(updInfo: Partial<Data>, treatmentDirty = false, exclude?): Observable<boolean> {
    ...
    if (patient) {
      this.Patient.next(updInfo ? Object.assign({}, this.Patient.getValue(), updInfo) : null);
      if (updInfo && updInfo.hasOwnProperty('triage')) this.patientDataService.updateVisitData({ triage: updInfo.triage });
      if (updInfo && updInfo.hasOwnProperty('patientHealthHistory')) this.patientDataService.updateVisitData({ patientHealthHistory: updInfo.patientHealthHistory });
  ...
  public getPatientIllness(): {[key in 'illness' | 'illnessName']: string[]} & { primary: string } {
    const illness = [];
    const illnessName = [];
    let primary: string;
  ...
  private getMedications(id: string) {
    const proceduresSub = this.proceduresService.getProcedures('medications')
      .subscribe(response => {
        const patient = this.getPatientLastValue();
  ...
  private getInjections(id: string) {
    const proceduresSub = this.proceduresService.getProcedures('injections')
      .subscribe(response => {
        const patient = this.getPatientLastValue();
   ...
   refreshPhysicalExam(examType: string) {
    this.getPhysicalExams(this.patientDataService.patientId.toString(), examType);
  }
  ...
  private getPhysicalExams(id: string, examType?: string) {
    ...
    const physicalExamsSub = this.kludges.getPhysicalExams(id).subscribe((data: PhysicalExamCover) => {
      if (!data) return;
      const patient = this.getPatientLastValue();
      const results = [];
      this.doctor_id = null;
      ...
      try {
        ...
        data.items.forEach(exam => {
          let examFound = results.find(item => item.examName === exam.examName);
          if (!examFound) {
            examFound = {
          ...
          R.sort((a, b) => a.left - b.left, exam.addedExams).forEach(item => {
            const normal = item.modifier.split(',')[0] === 'Normal';
            const examFinding = item.modifier.replace(normal ? 'Normal, ' : 'Abnormal, ', '');
            const foundedSystem = (this.userService.isPICModeBusiness && this.telemedPatient ? telemedExamPIC : physicalExamDesc )
              .find(desc => desc.examName === exam.examName);
            const foundedSubsystem = foundedSystem ? foundedSystem.descriptionsArray.find(desc => desc.description === exam.examResults) : undefined;
            const PEDElement = foundedSubsystem ? foundedSubsystem.mod.find(mod => mod.key === examFinding) : undefined;
            if (PEDElement) {
        ...
        if (this.userService.isPICModeBusiness && this.telemedPatient) {
          this.store.dispatch(new AddSystemsToPanel(telemedExamPIC.filter(item => item.examName !== (patient.patientInformation.gender === 'Male' ? 'Female GU' : 'Male GU')).map(item => {
            const system = R.clone(item);
            system.descriptionsArray = system.descriptionsArray.map(desc => {
              desc.mod = desc.mod.map(mod => Object.assign(mod, {edited: false, selected: false}));
              return Object.assign(desc, {edited: false, selected: false});
            });
            return system;
          })));
        } else {
          this.store.dispatch(new AddSystemsToPanel(physicalExamDesc.filter(item => item.examName !== (patient.patientInformation.gender === 'Male' ? 'Female GU' : 'Male GU')).map(item => {
            const system = R.clone(item);
            system.descriptionsArray = system.descriptionsArray.map(desc => {
              desc.mod = desc.mod.map(mod => Object.assign(mod, {edited: false, selected: false}));
              return Object.assign(desc, {edited: false, selected: false});
        ...
        results.forEach((physicalExams: PhysicalExam) => {
          physicalExams.addedExams.forEach(exam => {
            this.store.dispatch(new SelectFinding({key: exam.modifier.key}, physicalExams.examName, exam.description, true));
            if (exam.edited) this.store.dispatch(new EditFinding({text: exam.modifier.text, key: exam.modifier.key}, physicalExams.examName, exam.description));
          });
        });

        patient.mediaReviewed.Edited = !results.length;
        this.physicalExamResultViewModel$.pipe(
          first()).subscribe((physicalExams: PhysicalExam[]) => this.setInitialPhysicalExams(physicalExams));


        let allReviewed = true;
        const difference = DateTime.diff(new Date(), new Date(this.updated_at));
        for (const exam in patient.mediaReviewed) {
          if (this.updated_at !== null &&
            this.doctor_id !== null && (this.doctor_id === this.userService.getUserData.doctor_id)
            && (difference.hours <= 24)) {
  ...
  private getSelectedDiagnosis(patient: DataModel.Data) {
    ...
    const parseIllnesses = (illnesses: IllnessesInformation) => {
      if (!illnesses.is_edited_by_doctor && R.isNil(patient.additionalInformation.definedIcdCodes)) {
        patient.additionalInformation.definedIcdCodes = [];
      }

      illnesses.is_edited_by_doctor = true;
      illnesses.selected_illnesses.forEach(item => {
        const DE = [...patient.diagnosticEngine, ...patient.unlikelyDiagnosticEngine];
      ...
      if (this.userService.getUserRole === UserRolesEnum.PHARMACIST) this.stateService.patient.tabs.setWorkingDiagnosis(DiagnosisTabsEnum.PHARMACIST_SUMMARY);

      setTimeout(() => this.store.dispatch(new InitAccordion([...patient.diagnosticEngine, ...patient.unlikelyDiagnosticEngine.map(item => ({...item, source: 'UnlikelyDiagnosisToConsider'}))], patient.illnessInformation, patient.triage)).subscribe(() => {
        this.treatmentsService.restoreTreatments(patient, this.getPatientIllness().illness, this.getPatientIllness().primary).then(() => this.isPatientLoaded.next(true));
      }));

      continueSetupProcess(patient);
    };
    this.kludges.getSelectedDiagnosis(patient.patientInformation.patientId).subscribe((illnessesInformation: IllnessesInformation) => {
  ...
  private subscribeToTreatmentsDataUpdate(): void {
    this.treatmentsService.observe((treatmentsState) => {
      const patient = this.getPatientLastValue();
      if (!patient) return;
      const additionalInformation: PatientAdditionalInformation = {
        ...patient.additionalInformation,
        knownDrugConflicts: treatmentsState.drugConflictInformation.knownDrugConflicts,
        knownDrugAllergies: treatmentsState.drugConflictInformation.knownDrugAllergies,
        knownPrecautionConflicts: treatmentsState.drugConflictInformation.knownPrecautionConflicts
      };
      return {
      ...
      takeUntil(this.getPatient().pipe(skip(1), first(data => {
        return data === null;
      }))),
      filter(treatmentsState => !!treatmentsState)
    ).subscribe(result => this.updatePatient(result, result.treatmentsDirty));
  }

  private getSummary(data: Data) {
    const patient_id = data.patientInformation.patientId;
    this.kludges.getSummary(patient_id)
      .pipe(
        catchError(() => ObservableOf({summary: '', doctor: null})),
        concatMap(response => {
          if (response.summary.length) return ObservableOf(response);
          else
            return (this.userService.getUserRole === UserRolesEnum.PRACTITIONER || this.userService.getUserRole === UserRolesEnum.PHARMACIST) ?  this.kludges.generateSummary(data).pipe(catchError(() => ObservableOf({summary: ''})))
              : ObservableOf({
                summary: '',
                doctor: null
              });
  ...
  public updateDifferentialDiagnosis(generateDDX = true): void {
    if (generateDDX) {
      this.kludges.getDifferentialDiagnosis().pipe(
        map(ddx => R.isEmpty(ddx) ? {DDXRawAnalysis: {}, DDXCount: null, DDXSummary: ''} : ddx),
        catchError(() => ObservableOf({DDXRawAnalysis: {}, DDXCount: null, DDXSummary: ''}))
      ).subscribe(ddx => {
        this.differentialDiagnosis.next(R.isEmpty(ddx.DDXSummary) ? null : ddx);
      });
    } else {
  ...
  public saveHealthHistory(patient_id: string, update: boolean): Observable<any> {
    const patient = this.getPatientLastValue();
    return this.kludges.saveHealthHistory(patient_id, patient, update);
  }

  public closePatient(patient_id: string, data: any): Observable<{detail: string}> {
    return this.kludges.closePatient(patient_id, data);
  }


  public saveAdditionalInfo(patient_id: string, additionalInfo: AdditionalDoctorNotes, update: boolean): Observable<any> {
  ...
  private handleVisitDataContext(visitData: Data): void {
    const contextData = zipObj(keysIn(visitData), valuesIn(visitData).map(field => {
      if (typeof field !== 'object') return typeof field;
      switch (true) {
        case Array.isArray(field): return 'array';
        case isNil(field): return 'null';
  ...
  public get visitDataContext() { return this._visitDataContext.getValue(); }
}
