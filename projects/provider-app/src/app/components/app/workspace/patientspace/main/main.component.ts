...
export class MainComponent
  ...
  implements OnInit, OnDestroy, BottomButtonsControl {
  ...
  public illnessPresentation =
    IllnessPresentationTypesEnum[
      this.userService.getUserData.environment.illness_presentation
    ];
  ...
  public checklist = [
    {
      key: ChecklistEnum.SYMPTOMS,
      title: "General Symptoms",
      reviewed: false,
      visible: true,
    },
    {
      key: ChecklistEnum.PHYSICAL_EXAMS,
      title: "Physical Exam / Media",
      reviewed: false,
      visible: false,
    },
  ];
  ...
  public currentSort: {
    key: IllnessPresentationTypesEnum;
  ...
  @Select(DiagnosisAccordionState.diagnosisAccordion)
  storeAccordion: Observable<DiagnosisAccordionGroup[]>;
  @Select(DiagnosisAccordionState.definedICDList) storeDefinedICDs: Observable<
    DiagnosticEngine[]
  >;
  @Select(DiagnosisAccordionState.selectedIllnesses)
  storeSelectedIllnesses: Observable<SelectedIllness[]>;
  @Select(DiagnosisAccordionState.expandedElements)
  storeExpandedElements: Observable<{ [key: string]: boolean }>;
  @Select(DiagnosisAccordionState.diagnosticEngine)
  storeDiagnosticEngine: Observable<DiagnosticEngine[]>;

  public diagnosticEngine: DiagnosticEngine[] = [];
  public definedEngine$: Observable<
    DiagnosticEngine[]
  > = this.storeDiagnosticEngine.pipe(
    map((DE) => DE.filter((item) => item.icdGroup === "Defined Illnesses"))
  );

  public accordionGroups$: Observable<{
    [icdGroup: string]: DiagnosticEngine[];
  }>;
  ...
  public otherPossibilitiesGroup$: Observable<{
    [icdGroup: string]: DiagnosticEngine[];
  }>;
  public sortedOtherPossibilitiesGroup$: Observable<string[]>;

  public groupedContributor$: Observable<{ [icdGroup: string]: Contributor[] }>;
  public selectedIllnesses: Observable<
    DiagnosticEngine[]
  > = this.storeDiagnosticEngine.pipe(
    filter((data) => !!data),
    map((data) =>
      data.filter(
        (item) => item.isSelected && item.icdGroup !== "Defined Illnesses"
      )
    ),
    tap((data) =>
      this.stateService.patient.setIsPrimary(
        data.some((item) => item.isPrimary)
      )
    )
  );
  public definedIllnesses: Observable<
    DiagnosticEngine[]
  > = this.storeDiagnosticEngine.pipe(
    filter((data) => !!data),
    map((data) => data.filter((item) => item.icdGroup === "Defined Illnesses"))
  );
  ...
  get hasRefillRequests(): boolean {
    ...
    const needsRefill = historyItems.some(
      (item) => item.details && item.details.needsRefill
    );
    return needsRefill;
  }
  ...
  constructor(
    ...
    super(userService);
    this.currentSort = {
      key: this.illnessPresentation,
      direction:
        this.illnessPresentation === IllnessPresentationTypesEnum.CONFIDENCE
          ? "desc"
          : "asc",
    };
    this._edited
      ...
      .pipe(
        ...
        pairwise(),
        map(
          ([prev, current]: boolean[]): boolean =>
            (!this.viewOnly && current) || (prev && !current)
        )
      )
      .subscribe((val) => this._rerunTETrigger.next(val));
  }
  ...
  showFullImg(value, video, index) {
    ...
    this.dialogService.custom(
      PpPeMediaComponent,
      { ...dialogData, bodyPart, viewOnly: this.viewOnly },
      null,
      "viewmedia"
    );
  }
  ...
  toBodyPart(name) {
    this.navigationService.navigate(
      ["patients", this.activeRoute.parent.snapshot.params["id"], "vm"],
      { fragment: name }
    );
  }

  getPhysicalExamLink() {
    return ["/patients", this.activeRoute.parent.snapshot.params["id"], "vm"];
  }

  ngOnInit() {
    this.accordionGroups$ = this.store.select(
      DiagnosisAccordionState.accordionGroups(
        this.topIllnessQuantity,
        compose(not, DiagnosisAccordionState.isChronicOrCritical)
      )
    );
    this.sortedGroup$ = zip(
      this.store.select(
        DiagnosisAccordionState.accordionSortedGroupsNames(
          this.currentSort,
          compose(not, DiagnosisAccordionState.isChronicOrCritical)
        )
      ),
      ...
      map(([groupNames, groups]) =>
        groupNames.filter((item) => Boolean(groups[item]))
      )
    );

    this.chronicGroup$ = this.store.select(
      DiagnosisAccordionState.accordionGroups(
        this.topIllnessQuantity,
        DiagnosisAccordionState.isChronic
      )
    );
    this.sortedChronicGroup$ = zip(
      this.store.select(
        DiagnosisAccordionState.accordionSortedGroupsNames(
          this.currentSort,
          DiagnosisAccordionState.isChronic,
    ...
    this.otherPossibilitiesGroup$ = this.store.select(
      DiagnosisAccordionState.accordionGroups(
        this.topIllnessQuantity,
        DiagnosisAccordionState.isCritical
      )
    );
    this.sortedOtherPossibilitiesGroup$ = zip(
      this.store.select(
        DiagnosisAccordionState.accordionSortedGroupsNames(
          this.currentSort,
          DiagnosisAccordionState.isCritical,
    ...
    this.groupedContributor$ = this.store
      .select(DiagnosisAccordionState.groupedContributors)
      .pipe(map((data) => R.clone(data)));

    this.subs.push(
      this.storeAccordion
        .pipe(filter((data) => !R.isNil(data)))
        .subscribe((accordion) => {
    ...
    this.subs.push(
      this.storeDiagnosticEngine
        .pipe(filter((data) => !R.isNil(data)))
        .subscribe((DE) => {
          this.diagnosticEngine = DE;
          const illness = DE.filter((item) => item.isSelected).map(
            (item) => item.icd10
          );
          ...
          this.dataService.updatePatient({
            diagnosticEngine: DE,
            illnessInformation: {
              defined_illnesses: DE.filter(
                (item) =>
                  item.icdGroup === "Defined Illnesses" && item.isSelected
              ).map((item) => ({ code: item.icd10 })),
              selected_illnesses: DE.filter(
                (item) =>
                  item.isSelected && item.icdGroup !== "Defined Illnesses"
              ).map((item) => ({
    ...
    this.stateService.patient
      .getViewOnly()
      .subscribe((viewOnly: boolean) => (this.viewOnly = viewOnly));
    this.titleService.setTitle(this.title);
    this.subs.push(
      this.symptomsService
        .getParsedSymptoms()
        .pipe(filter((data) => !!data))
        .subscribe((parsedSymptoms) => {
          this.subs.push(
            this.dataService.getPatient().subscribe((patient) => {
              if (patient) {
                ...
                this.historyCardSections.personalHistory.list = parsedSymptoms.filter(
                  (item) =>
                    item["categoryID"] === HistoryTypesEnums.PERSONAL &&
                    item.symptomID !== HistoryTypesEnums["PREVIOUS-HISTORY"]
                );
                this.historyCardSections.personalHistory.data = historyItems.filter(
                  (item) =>
                    this.historyCardSections.personalHistory.list.find(
                      (listItem) => listItem.symptomID === item.symptomID
                    )
                );
                this.historyCardSections.drugAllergies.data = historyItems.filter(
                  (item) =>
                    item.symptomID === HistoryTypesEnums["MEDICATION-ALLERGIES"]
                );
              ...
              if (patient && !R.equals(this.Patient, patient)) {
                ...
                this.pharmacistSymptomsList = this.symptomsService.pharmacistSymptomsList(
                  this.Patient.triage
                );
                if (this.viewOnly) {
                  this.setChecklistItemReviewed(ChecklistEnum.SYMPTOMS);
                  this.setChecklistItemVisibility(
                    ChecklistEnum.PHYSICAL_EXAMS,
                    Boolean(this.images.length || this.audio.length)
                  );
                } else {
                  this.setChecklistItemVisibility(
                    ChecklistEnum.PHYSICAL_EXAMS,
          ...
          this.images = [];
          this.audio = [];
          const availableImages = this.imagesParts.filter(
            (part) => this.specificMediaAvailable[part]
          );
          const availableAudios = this.audioParts.filter(
            (part) => this.specificMediaAvailable[part]
          );
          ...
          this.subs.push(
            this.stateService.patient.getReviewed().subscribe((reviewed) => {
              this.reviewed =
                reviewed === "reviewed" || reviewed === "confirmed";
          ...
          this.subs.push(
            this.stateService.patient
              ...
              .subscribe((reviewed) => {
                ...
                if (reviewed)
                  this.setChecklistItemReviewed(ChecklistEnum.PHYSICAL_EXAMS);
              })
          );
        })
    );
  ...
  private prepareDifferentialDiagnosisForRequest(): void {
    this.subs.push(
      this.dataService.getDifferentialDiagnosis().subscribe((ddx: DDX) => {
        this.ddxRequest = [];
        if (ddx) {
          Object.keys(ddx).map((icdCode) =>
            this.ddxRequest.push(
              PatientdataService.createDDXForRequest(icdCode, ddx[icdCode])
            )
          );
        }
      })
  ...
  save(saveIllness = true): Observable<any> {
    ...
    return this.dataService
      .saveDiagnosis(payload, this.Patient.patientInformation.patientId)
      .pipe(finalize(() => (this.edited = false)));
  }
  ...
  reviewDialog(icd?, action?) {
    const reviewType = !this.reviewed
      ? !this.examReviewed
        ? "ReviewBoth"
        : "ReviewSymptoms"
      : "ReviewExams";
  ...
  toReview(icd?, action?, link = "symptoms") {
    ...
    if (icd)
      this.dataService.updatePatient({
        reviewIllness: { icd: icd, action: action },
      });
  }
  ...
  getSelected() {
    const response = this.diagnosticEngine.filter((de) =>
      this.selectedIllness.find((item) => item.icd10_code === de.icd10)
    );
  ...
  private showSelectedIllnesses(): void {
    if (this.stateService.app.ddx()) {
      ...
      this.save()
        .pipe(tap(() => this.dataService.updateDifferentialDiagnosis()))
        .subscribe();
      return;
  ...
  openConfirmDialog() {
    const illnessName = this.selectedIllness
      .map((item) => item.icd10_name)
      .concat(this.definedICDs.map((item) => item.icdName));
  ...
  private setChecklistItemVisibility(
  ...
  setPrimary(row: DiagnosticEngine, event) {
    ...
    if (this.reviewed && this.examReviewed) {
      const config = this.dialogSubscribesService.getConfig({
        message: `Change primary to ${row.icdName}?`,
        isDialog: true,
      });
      this.dialogService
        .open(PpcustomdialogComponent, config)
        .subscribe((result) => {
          if (result) {
  ...
  public get disallowConfirm(): boolean {
    return (
      Boolean(!this.getSelected().length && !this.definedICDs.length) ||
      this.viewOnly
    );
  }
  ...
  getLink(link: string) {
    return ["/patients", this.activeRoute.parent.snapshot.params["id"], link];
  }
